...
public class MonsterSpawner : MonoBehaviour
{
	...
	private const float ActiveEnemyUpdateInterval = 0.15f;

	private const float LibraryUpdateInterval = 1f;

	private Coroutine _networkKillRoutine;

	private string _lastKillStatus = "主机/单机下可直接击杀；非主机会自动尝试网络辅助击杀。";
	...
	public string LastKillStatus => _lastKillStatus;
	...
	public void KillEnemy(Enemy enemy)
	{
		if ((UnityEngine.Object)(object)enemy == (UnityEngine.Object)null)
		{
			_lastKillStatus = "目标怪物已失效，击杀请求已取消。";
			return;
		}
		if (CanKillDirectly())
		{
			DirectKillEnemy(enemy);
			_lastKillStatus = "已通过主机权限执行原生击杀。";
			return;
		}
		if (TryStartNetworkAssistKill(enemy, out var status))
		{
			_lastKillStatus = status;
			return;
		}
		_lastKillStatus = status;
	}
	...
	public void KillAllEnemies()
	{
		...
		if (CanKillDirectly())
		{
			...
			_lastKillStatus = string.Format("已通过主机权限执行清怪，共处理 {0} 只怪物。", array.Length);
			return;
		}
		if (_networkKillRoutine != null)
		{
		...
		_lastKillStatus = string.Format("非主机模式：已开始按顺序尝试网络辅助清怪，共 {0} 只。", array.Length);
	}
